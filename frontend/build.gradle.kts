...
dependencies {
    ...
    val example = "com.example:example-core-node:0.0.1"
    implementation(example)
    jarsToExtractJs.add(example)
}
...
val copyLibJsFiles by tasks.creating {
    ...
    doLast {
        ...
        configurations["runtimeClasspath"].resolvedConfiguration.firstLevelModuleDependencies.forEach { resolvedDependency: ResolvedDependency ->
            if (jarsToExtractJs.contains(resolvedDependency.module.toString())) {
                resolvedDependency.children.forEach { matchedArtifacts.addAll(it.allModuleArtifacts) }
            }
        }
